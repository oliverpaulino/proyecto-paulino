...
interface CostoFormProps {
   initialData?: any;
   predefinedValues?: Partial<CreateCostoForm>;
...
export function CostoForm({ initialData, predefinedValues, onSubmit, onCancel, loading }: CostoFormProps) {
   ...
   const isDisabled = (field: keyof CreateCostoForm) => {
   ...
   async function handleSubmit(e: React.FormEvent) {
      ...
      if (!values.proyecto_id) return setError("Debe seleccionar un proyecto asociado al costo.");
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 py-2 px-3 overflow-y-auto">
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <textarea 
                    ...
                    placeholder="Ej: Compra de material directo para la obra" 
                />
            </div>
         ...
         <div className="flex gap-2 justify-end pt-4 border-t border-border mt-2">
            ...
            <Button type="submit" disabled={loading}>{loading ? "Guardando..." : "Guardar Costo"}</Button>
         </div>
      </form>
   );
}