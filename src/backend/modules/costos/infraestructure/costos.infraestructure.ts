import { Kysely } from "kysely";
import { DB } from "@/backend/database";
import {
   CreateCostoDTO,
   DeleteCostoDTO,
   Costo,
   ICostoRepository,
   UpdateCostoDTO,
} from "../domain/costos.domain";

export class KyselyCostoRepository implements ICostoRepository {
   constructor(private readonly db: Kysely<DB>) { }

    private buildCodigoReferencia(prefix: string, referencia: number): string {
      const ref = String(referencia).padStart(3, "0");
      return `${prefix}-${ref}`;
    }

   private mapToEntity(row: any): Costo {
      return Costo.create({
         ...row,
         codigoReferencia: this.buildCodigoReferencia("COS", row.referencia),
         orden_compra_codigo_referencia: row.orden_compra_codigo_referencia 
            ? this.buildCodigoReferencia("OC", row.orden_compra_codigo_referencia) 
            : null,
         proyecto_codigo_referencia: row.proyecto_codigo_referencia || null,
         monto_total: Number(row.monto_total),
         fecha: new Date(row.fecha),
         created_at: new Date(row.created_at),
         updated_at: new Date(row.updated_at),
         deleted_at: row.deleted_at ? new Date(row.deleted_at) : null,
      });
   }

   private safeParseReferencia(search: string): number | null {
      if (!search) return null;
      
      const text = search.trim().toUpperCase();
      
      if (text.startsWith("COS-")) {
         const numero = Number(text.slice(4));
         return Number.isNaN(numero) ? null : numero;
      }
      
      const numeroDirecto = Number(text);
      return Number.isNaN(numeroDirecto) ? null : numeroDirecto;
   }

   private buildBaseQuery(isDeleted: boolean, params?: any) {
      let query = this.db
         .selectFrom("costo")
         .innerJoin("proyecto", "proyecto.id", "costo.proyecto_id")
         .leftJoin("orden_compra", "orden_compra.id", "costo.orden_compra_id")
         .selectAll("costo")
         .select([
            "orden_compra.referencia as orden_compra_codigo_referencia",
            "proyecto.id as proyecto_codigo_referencia",
         ]);

      if (isDeleted) {
         query = query.where("costo.deleted_at", "is not", null);
      } else {
         query = query.where("costo.deleted_at", "is", null);
      }

      if (params?.search) {
         const searchString = String(params.search).trim();
         const searchLike = `%${searchString}%`;
         const refNumber = this.safeParseReferencia(searchString);

         query = query.where((eb) => {
            const conditions: any[] = [
               eb("costo.concepto", "ilike", searchLike),
               eb("costo.ncf", "ilike", searchLike),
            ];

            if (refNumber !== null) {
               conditions.push(eb("costo.referencia", "=", refNumber));
            }

            return eb.or(conditions);
         });
      }

      if (params?.start) query = query.where("costo.fecha", ">=", params.start);
      if (params?.end) query = query.where("costo.fecha", "<=", params.end);
      if (params?.proyecto_id) query = query.where("costo.proyecto_id", "=", params.proyecto_id);
      if (params?.orden_compra_id) query = query.where("costo.orden_compra_id", "=", params.orden_compra_id);

      return query;
   }

   async findAll(params?: any): Promise<Costo[]> {
      const { page = 1, limit = 20 } = params || {};
      const rows = await this.buildBaseQuery(false, params)
         .orderBy("costo.fecha", "desc")
         .offset((page - 1) * limit)
         .limit(limit)
         .execute();

      return rows.map((row) => this.mapToEntity(row));
   }

   async findAllDeleted(params?: any): Promise<Costo[]> {
      const { page = 1, limit = 20 } = params || {};
      const rows = await this.buildBaseQuery(true, params)
         .orderBy("costo.deleted_at", "desc")
         .orderBy("costo.fecha", "desc")
         .offset((page - 1) * limit)
         .limit(limit)
         .execute();

      return rows.map((row) => this.mapToEntity(row));
   }

   async findById(id: string): Promise<Costo | null> {
      const row = await this.db
         .selectFrom("costo")
         .innerJoin("proyecto", "proyecto.id", "costo.proyecto_id")
         .leftJoin("orden_compra", "orden_compra.id", "costo.orden_compra_id")
         .selectAll("costo")
         .select([
            "orden_compra.referencia as orden_compra_codigo_referencia",
            "proyecto.id as proyecto_codigo_referencia",
         ])
         .where("costo.id", "=", id)
         .where("costo.deleted_at", "is", null)
         .executeTakeFirst();

      if (!row) return null;
      return this.mapToEntity(row);
   }

   async findDeletedById(id: string): Promise<Costo | null> {
      const row = await this.db
         .selectFrom("costo")
         .innerJoin("proyecto", "proyecto.id", "costo.proyecto_id")
         .leftJoin("orden_compra", "orden_compra.id", "costo.orden_compra_id")
         .selectAll("costo")
         .select([
            "orden_compra.referencia as orden_compra_codigo_referencia",
            "proyecto.id as proyecto_codigo_referencia",
         ])
         .where("costo.id", "=", id)
         .where("costo.deleted_at", "is not", null)
         .executeTakeFirst();

      if (!row) return null;
      return this.mapToEntity(row);
   }

   async create(data: CreateCostoDTO): Promise<Costo> {
      const row = await this.db
         .insertInto("costo")
         .values({
            proyecto_id: data.proyecto_id,
            monto_total: data.monto_total,
            concepto: data.concepto,
            ncf: data.ncf,
            orden_compra_id: data.orden_compra_id ?? null,
            fecha: data.fecha ?? new Date(),
            created_at: new Date(),
            updated_at: new Date(),
         })
         .returningAll()
         .executeTakeFirstOrThrow();

      return this.findById(row.id) as Promise<Costo>;
   }

   async update(id: string, data: UpdateCostoDTO): Promise<Costo | null> {
      await this.db
         .updateTable("costo")
         .set({ ...data, updated_at: new Date() })
         .where("id", "=", id)
         .where("deleted_at", "is", null)
         .executeTakeFirst();

      return this.findById(id);
   }

   async delete(id: string, data: DeleteCostoDTO): Promise<boolean> {
      const result = await this.db
         .updateTable("costo")
         .set({
            deleted_at: new Date(),
            deleted_by: data.deleted_by ?? null,
            deleted_reason: data.deleted_reason ?? null,
         })
         .where("id", "=", id)
         .where("deleted_at", "is", null)
         .executeTakeFirst();

      return Number(result.numUpdatedRows) > 0;
   }

   async restore(id: string): Promise<Costo | null> {
      const result = await this.db
         .updateTable("costo")
         .set({
            deleted_at: null,
            deleted_by: null,
            deleted_reason: null,
         })
         .where("id", "=", id)
         .where("deleted_at", "is not", null)
         .executeTakeFirst();

      return this.findById(id);
   }
}