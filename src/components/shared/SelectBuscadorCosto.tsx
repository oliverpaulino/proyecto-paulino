...
interface SelectBuscadorCostoProps {
   value?: string | null;
   initialLabel?: string;
   onChange: (costoId: string | null) => void;
   placeholder?: string;
   disabled?: boolean;
}

export function SelectBuscadorCosto({
   value,
   initialLabel = "",
   onChange,
   placeholder = "Buscar costo por concepto o NCF...",
   disabled = false,
}: SelectBuscadorCostoProps) {
   const { Costos, loading, GetCostos } = useCostoStore();
   ...
   }, [debouncedSearch, isOpen, GetCostos]);

   const handleSelect = (costo: Costo) => {
      setInputValue(`${costo.codigoReferencia} - ${costo.concepto}`);
      onChange(costo.id);
      setIsOpen(false);
   };
         ...
         {isOpen && (
            <div className="absolute z-50 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-border bg-popover text-popover-foreground shadow-md p-1">
               {loading && Costos.length === 0 ? (
                  <div className="p-4 text-center text-sm text-muted-foreground">Buscando costos...</div>
               ) : Costos.length > 0 ? (
                  Costos.map((costo) => (
                     <div
                        key={costo.id}
                        onClick={() => handleSelect(costo)}
                     ...
                     >
                        <div className="flex justify-between items-start gap-2">
                           <div className="flex flex-col overflow-hidden">
                              <span className="font-medium truncate text-foreground">{costo.concepto}</span>
                              <span className="text-xs text-muted-foreground">
                                 {costo.codigoReferencia} • NCF: {costo.ncf}
                              </span>
                           </div>
                           <span className="text-xs font-semibold whitespace-nowrap bg-secondary text-secondary-foreground px-1.5 py-0.5 rounded">
                              {new Intl.NumberFormat('es-DO', { style: 'currency', currency: 'DOP' }).format(costo.monto_total)}
                           </span>
                        </div>
                     </div>
                  ))
               ) : (
                  <div className="p-4 text-center text-sm text-muted-foreground">No se encontraron costos.</div>
               )}
            </div>
         )}
      </div>
